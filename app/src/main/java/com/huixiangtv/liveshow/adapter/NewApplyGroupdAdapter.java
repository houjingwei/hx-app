 ...
 */
public class NewApplyGroupdAdapter extends BaseAdapter {
    ...
    public NewApplyGroupdAdapter(Activity activity) {
        this.activity = activity;
        this.context = activity;
    }

    public NewApplyGroupdAdapter() {
    }
    ...
    private void replyAddFriend(final Friend friend) {
        ...
        params.put("gid",friend.getGid());
        RequestUtils.sendPostRequest(Api.REPLY_ADD_GROUP, params, new ResponseCallBack<Friend>() {
            @Override
            public void onSuccess(Friend data) {
                super.onSuccess(data);
                CommonHelper.showTip(context,"已成功入群");
                new Handler().postDelayed(new Runnable() {
                    public void run() {
                        ((NewApplyGroupActivity)activity).refresh();
                    }
                }, 1000);
            }
